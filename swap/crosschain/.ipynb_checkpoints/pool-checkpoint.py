...
def swap(source: int, target: int, amount: int, state: PoolState):
    assert (source != target), f"expected source != target"
    prev_state = deepcopy(state)
    old_state = deepcopy(state)
    prev_fee = calculate_fee(prev_state)

    state.assets_balances[source] += amount
    state.assets_balances[target] -= amount

    target_amount = amount
    for i in range(32):
        fee = calculate_fee(state)
        fee_diff = fee - prev_fee
        temp_amount = amount - fee_diff
        if abs(target_amount - temp_amount) < 0.01:
            ...
            state.assets_balances[source] = old_state.assets_balances[source] + amount
            # state.assets_total_supply += amount
            validate_allocation(prev_state, state)
